<!-- Mitra Ratings Page (grouped) -->
<!-- Migrated content from resources/views/livewire/mitra/ratings.blade.php -->
            ...
            <div class="space-y-4">
                @foreach($ratings as $rating)
                    <div class="bg-white rounded-2xl shadow-md p-5 hover:shadow-lg transition">
                        ...
                        <!-- Help Title -->
                        <p class="text-sm font-semibold text-gray-700 mb-2">{{ $rating->help->title }}</p>

                        <!-- Rating Comment -->
                        @if($rating->comment)
                            <p class="text-sm text-gray-600 leading-relaxed mb-3">{{ $rating->comment }}</p>
                        @endif

                        <!-- Rating Badge -->
                        <div class="flex gap-2">
                            <span
                                class="inline-block bg-primary-100 text-primary-700 px-3 py-1 rounded-full text-xs font-semibold">
                                {{ $rating->rating }}.0 dari 5
                            </span>
                            @if($rating->rating >= 4)
                            ...
                            @elseif($rating->rating >= 3)
                                <span
                                    class="inline-block bg-yellow-100 text-yellow-700 px-3 py-1 rounded-full text-xs font-semibold">
                                    ➜ Biasa
                                </span>
                            @else
        ...
        @else
            ...
            <div class="bg-white rounded-2xl p-10 text-center shadow-md">
                ...
                <p class="text-sm text-gray-600">
                    Mulai ambil bantuan untuk mendapatkan rating dari customer
                </p>
                <a href="{{ route('mitra.dashboard') }}?tab=tersedia"
                    class="inline-block mt-4 bg-primary-500 text-white px-6 py-2.5 rounded-lg font-bold text-sm hover:bg-primary-600 transition">
