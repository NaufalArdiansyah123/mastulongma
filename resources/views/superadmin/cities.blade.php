<div>
        ...
        <div class="grid grid-cols-2 gap-6 mb-8">
            <div class="bg-white rounded-2xl shadow-md p-6 border border-gray-200">
                <label class="block text-sm font-semibold text-gray-700 mb-3">
                    <svg class="w-5 h-5 inline mr-2 text-primary-600" fill="none" stroke="currentColor" <div
    ...
    @if($showModal)
        <div class="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-40">
            <div
                class="bg-white rounded-2xl shadow-xl w-11/12 sm:w-10/12 lg:w-3/4 max-w-5xl p-6 max-h-[95vh] overflow-y-auto">
                <div class="flex items-center justify-between mb-4">
                    <h3 class="text-lg font-semibold">{{ $editMode ? 'Edit Kota' : 'Tambah Kota' }}</h3>
                ...
                <div class="space-y-4">
                    ...
                    <div>
                        <label class="block text-sm font-medium text-gray-700">Admin Kota (opsional)</label>
                        <select wire:model.defer="admin_id" class="mt-1 block w-full rounded-xl border-gray-300 shadow-sm">
                            <option value="">-- Tidak ada --</option>
                            @foreach($admins as $admin)
                                <option value="{{ $admin->id }}">{{ $admin->name }} ({{ $admin->email }})</option>
                            @endforeach
                        </select>
                        @error('admin_id') <span class="text-red-600 text-sm">{{ $message }}</span> @enderror
                    </div>
                ...
                <div class="mt-6 flex justify-end space-x-3">
                    <button wire:click="$set('showModal', false)" class="px-4 py-2 bg-gray-100 rounded-xl">Batal</button>
                    <button wire:click.prevent="save" class="px-4 py-2 bg-primary-600 text-white rounded-xl">Simpan</button>
                </div>
            </div>
        </div>
    @endif
    ...
    @if($showDeleteModal)
        <div class="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-40">
            <div
                class="bg-white rounded-2xl shadow-xl w-11/12 sm:w-8/12 md:w-1/2 max-w-2xl p-6 max-h-[95vh] overflow-y-auto">
                <h3 class="text-lg font-semibold mb-4">Konfirmasi Hapus</h3>
                <p class="text-sm text-gray-600">Anda yakin ingin menghapus kota ini? Aksi ini tidak dapat dibatalkan.</p>
                <div class="mt-6 flex justify-end space-x-3">
                    <button wire:click="$set('showDeleteModal', false)"
                        class="px-4 py-2 bg-gray-100 rounded-xl">Batal</button>
                    <button wire:click.prevent="deleteCity"
                        class="px-4 py-2 bg-red-600 text-white rounded-xl">Hapus</button>
                </div>
            </div>
        </div>
    @endif
